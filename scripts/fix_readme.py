...
This project is designed as a portfolio-ready neurotechnology / BCI case study. It demonstrates not only model training, but also EEG preprocessing, validation leakage analysis, feature importance and model interpretation.
...
- **window-level cross-validation** — optimistic, can contain subject leakage;
- **subject-independent validation** — more realistic, test subjects are unseen during training.

The main finding is that window-level validation strongly overestimates performance.

## Datasets

The project uses two versions of STEW EEG workload data:
...
Subject-independent validation is substantially harder and gives lower, more realistic performance. This suggests that a large part of the window-level performance is likely driven by subject-specific EEG patterns rather than fully generalizable cognitive load markers.
