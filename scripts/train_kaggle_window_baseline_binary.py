...
def evaluate_model(model_name, model, X_features, y, groups, n_splits=5):
    ...
    for fold, (train_idx, test_idx) in enumerate(cv.split(X_features, y)):
        ...
        print("Train subjects:", len(train_groups))
        print("Test subjects:", len(test_groups))
        ...
        result = {
            "model": model_name,
            "fold": fold,
            "validation": "window_level_stratified_cv",
...
def main():
    ...
    for model_name, model in get_models().items():
        print("\n" + "=" * 80)
        print("Training window-level model:", model_name)
        print("=" * 80)
    ...
    results_path = reports_dir / "kaggle_window_binary_cv_results.csv"
    summary_path = reports_dir / "kaggle_window_binary_summary.csv"
